...
function CalloutNodeView({ node, updateAttributes, deleteNode }: any) {
  const rawVariant = node.attrs.variant || 'info';
  const variant: CalloutVariant = rawVariant in calloutConfig ? rawVariant : 'info';
  const config = calloutConfig[variant];

  const cycleVariant = useCallback(() => {
    const currentIndex = variants.indexOf(variant);
    const nextIndex = (currentIndex + 1) % variants.length;
    updateAttributes({ variant: variants[nextIndex] });
  }, [variant, updateAttributes]);

  return (
    <NodeViewWrapper>
      <div className={`${config.bg} ${config.border} ${config.text} border rounded-lg p-4 my-3 relative group`}>
        <div className="flex items-start gap-3">
          <button
            type="button"
            onClick={cycleVariant}
            className="flex-shrink-0 text-lg cursor-pointer hover:scale-110 transition-transform select-none"
        ...
        <button
          ...
          onClick={deleteNode}
          contentEditable={false}
          className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 transition-opacity p-1 rounded"
